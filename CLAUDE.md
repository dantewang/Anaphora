...
游戏机制（由用户给出）：多角色组队。普攻分多段，前几段轻击、最后一段重击。战斗中
重击 / 技能 / 连携 / 大招的施放顺序会极大影响攒大招的速度和伤害——所以这四项要一起看，
overlay 的价值在于让这个顺序决策不用靠记忆。

### 1. 技能点（全队共享，三段）

底部中央，蓝色血条正上方，约 x 1590–2250、y 1945–1970，三段等宽。

读法：**黄色 = 已攒满的整段，白色 = 正在填充的当前段**。沿三段横向采一条线，数满格
黄段得整数部分，当前白段的填充比例得小数部分，输出 0..3。每个技能消耗量不定，但单次
最多消耗一段，所以要能读出小数。
...
- **敌人血条**：锚在世界坐标跟着敌人走，不是固定 ROI，要先做检测，成本高一个量级。
- **技能图标状态**：见上。

### 两个必须处理的坑

- **HUD 会整个消失。** 战斗一结束右下技能区就是纯背景。必须先判断"当前有没有 HUD"，
  否则读出来全是噪声。廉价做法：拿一两个 HUD 特征点当哨兵。
- **特效会糊掉 HUD。** 连拍里拍到过整条状态栏被白光完全盖住的帧，也拍到过大招/连携
  发动时的大幅切入动画（左侧平行四边形角色特写）盖住左半屏。阈值不能卡死，需要
  "本帧不可信、沿用上一帧"的判定。
